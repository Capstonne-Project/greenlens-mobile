---
description: Guided feature development with codebase understanding and architecture focus. Accepts an optional plan file path: /execute plans/260418-auth/plan.md
---

# /excute — Structured Implementation Pipeline

## Usage

```
/execute [--auto | --fast | --parallel] [plan-file-path | feature-description]
```
...
When a plan file path is provided, report what will be executed:
...
### Step 3.5 — Security & Performance Pre-Check (before code-reviewer)

Before sending to code-reviewer, verify:

**Security (auto-check these):**
1. Search for `AsyncStorage` usage with token/auth keywords → FAIL if found
2. Search for `console.log` outside `if (__DEV__)` with user/token/password → WARN
3. Search for `http://` in API URL configs (not localhost) → FAIL if found
4. Check all new form components have Zod schema validation → WARN if missing

**Performance (auto-check these):**
1. New list components: check `React.memo` wrapping item component
2. New FlatList: check for `removeClippedSubviews`, `maxToRenderPerBatch`
3. New animations: check `useAnimatedStyle` not calling `setState`
4. New queries: check `staleTime` is set for list queries

If any FAIL check triggers → fix before proceeding to code-reviewer.
If only WARN → proceed but include in review notes.
...
Step 5 is **always required** — execute is incomplete without all 3 sub-agents:
...
- `/plan --hard <description>` — create a plan file first, then pass it to `/execute`
