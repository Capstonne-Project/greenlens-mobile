...
/**
 * Bước 2 của luồng quên mật khẩu: nhập mã OTP.
 *
 * Cố tình KHÔNG gọi `POST /auth/verify-otp` ở đây — handler đó `MarkUsed()` mã, mà
 * `POST /auth/reset-password` ở bước sau cần chính mã này. Verify trước sẽ làm bước sau
 * luôn fail `OTP_EXPIRED`. Màn này chỉ kiểm tra đủ 6 chữ số rồi chuyển mã sang bước đặt
 * mật khẩu; BE xác thực mã một lần duy nhất khi reset.
 */
export default function VerifyResetOtpScreen() {
  ...
  const handleContinue = () => {
    if (!email) {
      setError('Thiếu email — quay lại bước quên mật khẩu.');
      return;
    }
    if (!isComplete) {
  ...
  const dialogTop = getAuthDialogTop('verify-reset-otp', height);
