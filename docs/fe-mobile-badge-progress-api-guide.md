...
|------|---------|------------------|
| `BadgeEarned` | Vừa unlock badge | `/(tabs)/gamification/badges` |
| `BadgeProgressNear` | `current === target - 1` (gần đạt) | Cùng màn Badges, scroll tới badge |

Placeholder template: `{badgeName}`, `{current}`, `{target}`.
