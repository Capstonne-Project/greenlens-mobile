...
Spawn the **`rn-debugger`** agent with the scout evidence report:
...
### Step 2.5 — Security Regression Check (after applying fix)

Before marking fix complete, verify the fix doesn't introduce:
- [ ] New `console.log` with sensitive data
- [ ] Changed token storage from SecureStore to AsyncStorage
- [ ] Removed Zod validation from a form
- [ ] Added `http://` URL
- [ ] Exposed error details to UI (`error.message` directly rendered)
- [ ] Added inline heavy computation without `useMemo`

If any item above is true → flag to code-reviewer with HIGH priority.
...
| `rn-debugger`     | 2 — root cause + apply fix | All                                   |
...
- `/plan` → `/execute` → `/fix` — fix regressions found after executing
